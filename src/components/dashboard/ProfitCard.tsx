// components/dashboard/ProfitCard.tsx
...
interface DashboardStats {
    ...
    totalUsers: number;
    todaySales: number;
    totalProfit?: number; // New field we hope to get
    growth: {
        sales: number;
        transactions: number;
        products: number;
...
const ProfitCard: React.FC<ProfitCardProps> = ({
    ...
    const [profitData, setProfitData] = useState<number>(amount || 0);
    ...
    const fetchDashboardStats = async () => {
        try {
            ...
            if (result.success && result.data) {
                ...
                // Logic: Use totalProfit if available, otherwise calculate using a standard 15% margin as fallback
                // In a real scenario, the backend should be updated to return this.
                const profit = data.totalProfit ?? Math.round(data.totalSales * 0.15);
                setProfitData(profit);
                setGrowthPercentage(data.growth.profit ?? data.growth.sales);
            } else {
